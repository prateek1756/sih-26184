...
async def list_alerts(
    ...
    query = (
        select(Alert)
        .options(selectinload(Alert.prediction), selectinload(Alert.assignee))
    )
    if status:
...
async def get_alert_detail(
    ...
    result = await db.execute(
        select(Alert).options(selectinload(Alert.prediction), selectinload(Alert.assignee)).where(Alert.id == alert_id)
    )
