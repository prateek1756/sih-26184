...
async def trigger_prediction_run(
    ...
    # Re-fetch with eager loading to avoid MissingGreenlet on property getters
    pred_ids = [p.id for p in predictions[:50]]
    if pred_ids:
        reload_q = (
            select(RiskPrediction)
            .options(selectinload(RiskPrediction.atm))
            .where(RiskPrediction.id.in_(pred_ids))
            .order_by(desc(RiskPrediction.risk_score))
        )
        reload_res = await db.execute(reload_q)
        loaded = reload_res.scalars().all()
    else:
        loaded = []

    return StandardResponse(
        data=[RiskPredictionRead.model_validate(p) for p in loaded]
    )


@router.get("/hotspots/live", response_model=StandardResponse[List[RiskPredictionRead]])
async def get_live_hotspots(
    ...
    """
    Execute real-time ML inference across PostgreSQL ATMs and transactions.
    Authenticated read-only endpoint: accessible to INVESTIGATORS, ANALYSTS, and other roles.
    Does not require ML_ENGINEER privileges.
    """
    predictions = await MLInferenceService.run_live_inference_readonly(
        db=db,
        window_hours=24,
        city=city,
        limit=limit,
    )
    return StandardResponse(data=predictions)
