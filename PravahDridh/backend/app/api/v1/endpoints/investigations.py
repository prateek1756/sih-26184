...
async def create_investigation(
    ...
    new_inv = Investigation(
        ...
        complaint_id=inv_in.complaint_id,
...
@router.patch("/{investigation_id}/status", response_model=StandardResponse[InvestigationRead])
async def update_investigation_status(
    ...
    await AuditService.log_event(
        db=db,
        event_type="INVESTIGATION_STATUS_UPDATE",
        actor=current_user,
        resource_type="Investigation",
        resource_id=inv.id,
        action_details={"old_status": old_status, "new_status": status_in.status},
        ip_address=request.client.host if request.client else None,
    )
    await db.commit()
    await db.refresh(inv)
...
@router.post("/{investigation_id}/outcome", response_model=StandardResponse[InvestigationRead])
async def record_investigation_outcome(
    investigation_id: uuid.UUID,
    outcome_in: InvestigationOutcomeRequest,
    request: Request,
    current_user: User = Depends(require_roles(["INVESTIGATOR", "SUPERVISOR", "ADMIN"])),
    db: AsyncSession = Depends(get_db),
):
    """
    Record real investigation outcome feedback loop:
    - WITHDRAWAL_OCCURRED
    - NO_WITHDRAWAL_DETECTED
    - FALSE_POSITIVE
    - INTERVENTION_PREVENTED_CASHOUT
    """
    valid_outcomes = {
        "WITHDRAWAL_OCCURRED",
        "NO_WITHDRAWAL_DETECTED",
    ...
    if outcome_in.outcome not in valid_outcomes:
        raise HTTPException(
            status_code=400,
    ...
    inv.outcome = outcome_in.outcome
    if outcome_in.outcome_notes:
        inv.outcome_notes = outcome_in.outcome_notes
    if outcome_in.action_taken:
        inv.action_taken = outcome_in.action_taken
    inv.outcome_recorded_at = datetime.now(timezone.utc)
    inv.updated_at = datetime.now(timezone.utc)

    # Automatically set appropriate status based on outcome
    if outcome_in.outcome in {"RESOLVED", "INTERVENTION_PREVENTED_CASHOUT"}:
        inv.status = "RESOLVED"
    elif outcome_in.outcome == "FALSE_POSITIVE":
        inv.status = "FALSE_POSITIVE"

    await AuditService.log_event(
        db=db,
        event_type="INVESTIGATION_OUTCOME_RECORD",
        actor=current_user,
        resource_type="Investigation",
        resource_id=inv.id,
        action_details={
            "outcome": outcome_in.outcome,
            "action_taken": outcome_in.action_taken,
